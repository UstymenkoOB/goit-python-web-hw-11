...
async def read_contacts_name(name: str, db: Session = Depends(get_db)):
    contacts = await repository_contacts.get_contacts_name(name, db)
    if not contacts:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found")
    return contacts
...
async def read_contacts_surname(surname: str, db: Session = Depends(get_db)):
    contacts = await repository_contacts.get_contacts_surname(surname, db)
    if not contacts:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found")
    return contacts
...
async def read_contacts_email(email: str, db: Session = Depends(get_db)):
    contacts = await repository_contacts.get_contacts_email(email, db)
    if not contacts:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found")
    return contacts
